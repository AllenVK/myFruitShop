        ...
        <c:forEach items="${pagers.datas}" var="data" varStatus="1">
            <tr>
                ...
                <td style="color: red">
                    ...
                    <c:if test="${data.status == 3}">已签收</c:if>
                </td>
                <td>
                    <a class="button border-main" href="${ctx}/orderDetail/uList?orderId=${data.id}"><span class="icon-edit">查看订单</span> </a>
                    <c:if test="${data.status == 0}">
                        <a class="button border-red" href="${ctx}/itemOrder/fh?id=${data.id}"><span class="icon-car">去发货</span> </a>
                    </c:if>
                </td>
            </tr>
        </c:forEach>
